...
const CropDisease = () => {
  ...
  const analyzeImage = async () => {
    ...
    setAnalyzing(true);
    try {
      const res = await fetch("http://localhost:5000/api/analyze-crop-disease", {
        method: "POST",
  ...
  const severityColor = (s) => {
    if (s === "Low") return "text-green-500 bg-green-500/10";
    if (s === "Medium") return "text-yellow-500 bg-yellow-500/10";
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="container max-w-2xl py-6 px-4">
        ...
        <div className="bg-card rounded-xl shadow-sm border p-6 mb-6 animate-fade-up">
          ...
          {!imagePreview ? (
            <div className="border-2 border-dashed border-border rounded-xl p-12 text-center cursor-pointer hover:bg-primary/5 transition-colors" onClick={() => fileInputRef.current?.click()}>
        ...
        {result && (
          <div className="space-y-4 animate-fade-up">
            ...
            <div className="bg-card rounded-xl p-6 shadow-sm border">
              <div className="flex items-center gap-3 mb-4">
                <div className="w-10 h-10 rounded-full bg-green-500/10 flex items-center justify-center"><Shield className="h-5 w-5 text-green-500" /></div>
                <h2 className="text-lg font-bold">🛡️ Prevention Tips</h2>
              </div>
              <ul className="space-y-2">
                {result.prevention?.map((p, i) => (
                  <li key={i} className="flex gap-2 text-sm"><span className="text-green-500 font-bold">•</span><span className="text-muted-foreground leading-relaxed">{p}</span></li>
                ))}
              </ul>
            </div>
          </div>
