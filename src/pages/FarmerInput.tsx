          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            {fields.map(f => (
              <div key={f.id}>
                ...
                <Input id={f.id} type="number" step="any" placeholder={f.placeholder} className="text-sm py-5 bg-background" value={form[f.id as keyof typeof form]} onChange={e => update(f.id, e.target.value)} required />
              </div>
            ))}
